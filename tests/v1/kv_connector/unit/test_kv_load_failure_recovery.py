...
def test_deepseek_v4_load_failure_recomputes_whole_request(
    ...
    assert request.prefill_stats is not None
    assert request.prefill_stats.num_external_cached_tokens == 8 * 256
    ...
    # Finish the failed transfer, then recompute without reusing its invalid KV.
    deepseek_v4_scheduler.update_from_output(
        scheduler_output,
        create_model_runner_output(
            reqs=[], finished_recving={request.request_id}, use_eos=True
        ),
    )
    deepseek_v4_scheduler.connector.get_num_new_matched_tokens.side_effect = (
        _make_get_num_new_matched_tokens({}, async_load=False)
    )
    retry_output = deepseek_v4_scheduler.schedule()
    assert retry_output.num_scheduled_tokens[request.request_id] == request.num_tokens
    with patch.object(
        deepseek_v4_scheduler, "_connector_finished", return_value=(False, None)
    ):
        outputs = deepseek_v4_scheduler.update_from_output(
            retry_output, create_model_runner_output([request], use_eos=True)
        )
    output = next(iter(outputs.values())).outputs[0]
    assert output.finish_reason == FinishReason.STOP
    assert output.prefill_stats is not None
    assert output.prefill_stats.num_external_cached_tokens == 0
    assert output.prefill_stats.external_cached_token_sources == []
    assert output.prefill_stats.num_computed_tokens == request.num_prompt_tokens
