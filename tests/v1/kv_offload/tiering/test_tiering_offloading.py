...
class TestTieringOffloadingManager:
    ...
    def test_successful_promotion_finalizes_primary_with_success(self, manager_setup):
        ...
        with (
            ...
            patch.object(self.primary_tier, "complete_write") as completed,
            patch.object(self.primary_tier, "prepare_load") as pinned,
        ):
            self.manager._process_finished_jobs()
        completed.assert_called_once_with(to_keys([1]), _CTX, True)
        pinned.assert_called_once_with(to_keys([1]), _CTX)
...
class TestPromotionContentionProtection:
    ...
    def test_consumption_lifts_protection(self, manager_setup):
        ...
        assert self.manager.lookup(block_c, ctx_c) is LookupResult.HIT_PENDING
...
def test_pinned_promotion_count_is_capped():
    ...
    def land(block, ctx):
        assert manager.lookup(block, ctx) is LookupResult.HIT_PENDING
...
def test_always_promote_policy_never_pins():
    ...
    assert manager.lookup(block_a, ctx_a) is LookupResult.HIT_PENDING
    end_ctx = ScheduleEndContext(new_req_ids=[], preempted_req_ids=())
    manager.on_schedule_end(end_ctx)
    manager.on_schedule_end(end_ctx)
    ...
    ctx_b = ReqContext(req_id="req_b")
    secondary_tier.blocks[block_b] = True
    assert manager.lookup(block_b, ctx_b) is LookupResult.HIT_PENDING
